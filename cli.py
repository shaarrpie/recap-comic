...
Backends (--backend): gemini (default) | openai | anthropic | fixture | none.
...
def guided_plan(
    ...
    backend: str = typer.Option(
        "gemini", "--backend",
        help="gemini|openai|anthropic|fixture|none"),
    model: str | None = typer.Option(
        None, "--model",
        help="vision model id (required for openai/anthropic)"),
    ...
    debug_overlay: Path | None = typer.Option(
        None, "--debug-overlay",
        help="draw AI boundaries (red) + snapped gutters (green) + labels"),
    ...
    (green), panel IDs/confidence labels, and bubble boxes (blue).
    """
    ...
    try:
        plan, _artifact, _used = gp.run_guided(
            ...
            cache_dir=cache_dir, force=force, dry_run=True)
    ...
    if out_plan is not None:
        ...
        out_plan.write_text(plan.model_dump_json(indent=2) + "\n", "utf-8")
        typer.echo(f"plan written to {out_plan}")
    if debug_overlay is not None:
        from debug_view import draw_overlay
        draw_overlay(strip, plan, debug_overlay)
        typer.echo(f"debug overlay written to {debug_overlay}")
...
def guided_run(
    ...
    try:
        plan, artifact, used = gp.run_guided(
            ...
            variance_threshold=variance_threshold, fallback=fallback,
